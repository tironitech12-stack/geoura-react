...
import { applyInstallPlan, createInstallPlan, diagnoseInstallation, detectProject, registerInstallation } from "../src/installer.mjs";
...
async function init(args){
  ...
  const siteUrl=args.siteUrl||process.env.GEOURA_SITE_URL;
  if(send&&!publicKey){if(!siteUrl)throw new Error("Use --site-url https://seusite.com para registrar a instalação antes de ativar --send.");const registered=await registerInstallation({siteId,origin:siteUrl,apiUrl});publicKey=registered.publicKey;output.write(`Instalação registrada para ${registered.origin}.\n`);}
  const plan=await createInstallPlan({root:process.cwd(),siteId,apiUrl,publicKey,send});
...
function help(){output.write(`GEOura CLI\n\n  npx geoura init [--site-id id] [--api-url url] [--site-url url] [--public-key chave] [--send] [--yes] [--dry-run]\n  npx geoura doctor\n\nCom --send, defina GEOURA_ADMIN_TOKEN e --site-url para registrar automaticamente a chave pública. O token administrativo nunca é gravado no frontend.\n`);}
