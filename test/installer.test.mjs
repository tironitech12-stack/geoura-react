...
import { applyInstallPlan, createInstallPlan, diagnoseInstallation, registerInstallation } from "../src/installer.mjs";
...
test("registro automático usa token apenas no cabeçalho administrativo",async()=>{
  let request;
  const fetcher=async(url,options)=>{request={url,options};return {ok:true,json:async()=>({siteId:"site",origin:"https://example.com",publicKey:"gpk_real"})};};
  const result=await registerInstallation({siteId:"site",origin:"https://example.com/path",apiUrl:"https://api.example.com",adminToken:"secret",fetcher});
  assert.equal(result.publicKey,"gpk_real");assert.equal(request.options.headers.authorization,"Bearer secret");assert.equal(JSON.parse(request.options.body).origin,"https://example.com");assert.equal(request.options.body.includes("secret"),false);
});
